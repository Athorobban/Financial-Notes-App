    ...
    <Card className="w-full p-0 border-primary/20">
      <CardContent className="pr-2">
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex items-center gap-2">
            ...
            render={({ field }) => (
              <Field>
                <input
                  {...field}
                  id="form-message"
                  placeholder={isRecording ? "Listening..." : isPending && !field.value ? "Processing your request..." : "Manage your transaction here"}
                  autoComplete="off"
          ...
          />
          <Button type={isText ? "submit" : "button"} size="icon" variant="ghost" disabled={isPending} onClick={!isText ? (isRecording ? stopRecording : startRecording) : undefined}>
            {isPending ? <Loader2Icon className="size-5 animate-spin" /> : isText ? <SendIcon className="size-5" /> : isRecording ? <SquareIcon className="text-red-500 fill-red-500 size-5 animate-pulse" /> : <MicIcon className="size-5" />}
          </Button>
        </form>
      </CardContent>
    </Card>
