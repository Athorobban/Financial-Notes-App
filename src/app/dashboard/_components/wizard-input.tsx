      ...
      <CardContent className="pr-2">
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex items-center gap-2">
          ...
          <Controller
            ...
            render={({ field }) => (
              <Field>
                <input {...field} id="form-message" placeholder="Write your transaction here" autoComplete="off" className="h-14 focus:outline-none" onKeyDown={handleKeyDown} disabled={isPending} />
              </Field>
            )}
          />
          <Button type="submit" size="icon" variant="ghost" disabled={isPending}>
            {isPending ? <Loader2Icon className="size-5 animate-spin" /> : <SendIcon className="size-5" />}
          </Button>
        </form>
      </CardContent>
    </Card>
