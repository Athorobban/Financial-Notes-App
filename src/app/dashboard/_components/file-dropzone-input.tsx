...
export default function FileDropzoneInput({
  ...
  const processFile = async (file: File) => {
    if (!file.type.endsWith("pdf") && !file.type.startsWith("image") && !file.type.startsWith("video") && !file.type.startsWith("audio")) {
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
      className={cn("border-2 border-dashed rounded-xl p-6 cursor-pointer transition-all", isDragging ? "border-primary bg-primary/10 scale-[1.02]" : "border-muted hover:border-primary/50 hover:bg-muted/50")}
    >
      <input type="file" ref={fileInputRef} className="hidden" accept=".pdf, image/*, video/*, audio/*" onChange={(e) => e.target.files && processFile(e.target.files[0])} />
      {isPending ? (
