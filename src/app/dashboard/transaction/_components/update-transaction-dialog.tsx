...
export default function UpdateTransactionDialog({
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      amount: selectedTransaction ? String(selectedTransaction.data.amount) : "",
      ...
      description: selectedTransaction ? selectedTransaction.data.description : "",
    },
  });
  ...
  return (
    <Dialog open={!!selectedTransaction && selectedTransaction.action === "update"} onOpenChange={() => setSelectedTransaction(null)}>
      <DialogContent>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <DialogHeader className="gap-4">
            <div>
              <DialogTitle>Update Transaction</DialogTitle>
              <DialogDescription>Update the transaction data below.</DialogDescription>
            </div>
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field className="gap-1">
                    ...
                    <Input {...field} id="form-amount" placeholder="0,00" autoComplete="off" type="number" />
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field className="gap-1">
                    ...
                    <DatePicker id="form-date" value={field.value ? new Date(field.value) : undefined} onChange={(date) => field.onChange(date ? format(date, "yyyy-MM-dd") : "")} />
                    {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
                  </Field>
                )}
              />
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field className="gap-1">
                    ...
                    <Textarea {...field} id="form-description" placeholder="Enter description" autoComplete="off" />
                    {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
                  </Field>
                )}
              />
          ...
          <DialogFooter className="mt-4">
            <Button variant="ghost" onClick={() => setSelectedTransaction(null)} disabled={isPending}>
              Cancel
            </Button>
