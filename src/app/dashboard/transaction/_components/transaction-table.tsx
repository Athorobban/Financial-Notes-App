    ...
    <Fragment>
      <Card className="w-full gap-2">
        <CardHeader className="flex flex-col justify-between gap-2 md:flex-row md:items-center">
          ...
          <div>
            <Input placeholder="Search..." value={localSearch} onChange={(e) => setLocalSearch(e.target.value)} className="w-full" />
          </div>
        </CardHeader>
        <CardContent>
          <Table>
              ...
              {!isLoading &&
                transactions?.data?.map((transaction, index) => (
                  <TableRow key={`tr-${transaction.id}`}>
                    ...
                    <TableCell className={cn("font-semibold", transaction.type === "expense" ? "text-destructive" : "text-green-500")}>
            ...
            {isLoading && <TableCaption className="mb-4">Loading...</TableCaption>}
            {!isLoading && transactions?.data?.length === 0 && <TableCaption className="mb-4">No transactions found</TableCaption>}
          </Table>
      ...
      <DeleteTransactionDialog selectedTransaction={selectedTransaction} setSelectedTransaction={setSelectedTransaction} refetch={refetch} />
      <UpdateTransactionDialog selectedTransaction={selectedTransaction} setSelectedTransaction={setSelectedTransaction} refetch={refetch} />
    </Fragment>
  );
}
